    ...
    private fun ensureSessionIsSaved() {
        ...
        if (preferencesManager.rememberMe && !tokenManager.hasValidSession()) {
            ...
            if (currentUser != null) {
                Log.d(
                        "SuccessActivity",
                        "Remember Me was checked - saving session for auto-login: ${currentUser.email}"
                )
                ...
                Log.d(
                        "SuccessActivity",
                        "Auto-login session saved, expires in ${sessionDuration / (24 * 60 * 60 * 1000)} days"
                )
            }
        } else {
            Log.d(
                    "SuccessActivity",
                    "Remember Me not checked - user will need to login manually on next app start"
            )
        }
    }
