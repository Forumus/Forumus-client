 ...
 * - CommentItem: Shows comment content with voting and optional detail mode for nesting
 *
 ...
 * @param isDetailMode Boolean flag indicating whether to render comments in detail mode (with nesting)
 */
class ProfileAdapter(
    ...
    private val isDetailMode: Boolean = false
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (val item = items[position]) {
            ...
            is FeedItem.CommentItem -> (holder as CommentViewHolder).bind(item.comment, isDetailMode)
        }
    }
